...
@interface PayHistoryVC ()

@property(nonatomic,strong)payDetailVc *payD;

@end

@implementation PayHistoryVC
...
{
    PayHistoryCell *cell = [tableView dequeueReusableCellWithIdentifier:@"payHisCell"];
    if (!cell) {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"PayHistoryCell" owner:nil options:nil]lastObject];
    }
    if (!self.searchVC.active) {
    ...
    cell.lookDet = ^(){
        ...
        [self.navigationController pushViewController:_payD animated:YES];
    };
            return cell;
}
