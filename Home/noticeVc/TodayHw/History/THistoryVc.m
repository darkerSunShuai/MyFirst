...
#import "THistoryVc.h"
#import "THistoryCell.h"
#import "HomeWorkDetVc.h"

@interface THistoryVc ()

@end

@implementation THistoryVc
...
{
    THistoryCell *cell = [tableView dequeueReusableCellWithIdentifier:@"tHisCell"];
    if (!cell) {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"THistoryCell" owner:nil options:nil]lastObject];
    }
    if (!self.searchVC.active) {
        cell.dataL.text = [NSString stringWithFormat:@"%@",self.dataSource1[indexPath.row]];
        cell.classL.text = [NSString stringWithFormat:@"%@",self.dataSource2[indexPath.row]];
    }else
    {
        if (self.bt1) {
            cell.dataL.text = [NSString stringWithFormat:@"%@",self.searchArr[indexPath.row]];
        ...
        }else
        {
            ...
            cell.dataL.text = [NSString stringWithFormat:@"%@",self.dataSource1[i]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HomeWorkDetVc * homeWorkVc = [[HomeWorkDetVc alloc]init];
